...
class money{
	...
	public function get_money_limit(){
		...
		$result   = pdo_fetch("select * from ".tablename('lianhu_money_limit')." where  school_id=:sid and limit_module like :limit_module and  status=1  {$where_str} order by add_time desc  ",array(':sid'=>$_SESSION['school_id'],':limit_module'=>'%'.$this->do_action.'%' ));
		$this->money_limit = $result;
	}
	#获取最后一次缴费历史
	public function last_money_record(){
		$limit_id = $this->money_limit['limit_id'];
		$fanid    = D('base')->mobileGetFanidByUid($this->uid);
		$result   = pdo_fetch("select * from ".tablename("lianhu_money_record")." where  limit_id=:lid and status=1 and school_id=:sid and fan_id=:fan_id 
							  and student_id=:student_id order by addtime desc", array(':lid'=>$limit_id,':sid'=>$_SESSION['school_id'], ':fan_id'=>$fanid ,":student_id"=>$this->student_id  ));
