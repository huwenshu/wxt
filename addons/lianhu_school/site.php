...
class Lianhu_schoolModuleSite extends WeModuleSite {
	...
	public function registerSchoolInfo($school_id){
		...
		if($result['uniacid'] == $_W['uniacid']){
			...
			$this->where_uniacid_school = " uniacid=".$_W['uniacid']." and school_id=".$result['school_id']." ";                
			return true;			
		}else{
			return false;
		}
	...
	*/
    ...
    public function imgFrom($imgname){
        ...
        if(stristr($imgname,"qiniu")){
             $url =  $this->module['config']['qiniu_url'].$imgname;
        }else{
			 if(file_exists($_W['attachurl'].$imgname)){
				 $url =  $_W['attachurl'].$imgname;
			 }else{
				 $url = $_W['attachurl_local'].$imgname;
			 }
        }
		return $url;
    }
	...
	*/
    ...
    public function classTeacher($class_id){
        ...
        $result       = pdo_fetchall("select * from ".$this->table_pe."lianhu_teacher where 
                                      teacher_other_power like :power and status=1  {$other_where} ",array(':power'=>"%{$class_id}%"));    
        return $result;   
    }
    ...
    public function checkmobile($fanid){
			...
			if(!$hav){
				$this->myMessage("您还未绑定学生账号",$this->createMobileUrl('bangding'),'错误');
			}else{
				#已经绑定的情况下
	...
	public function teacher_mobile_qx($no = false){
		...
		$result = pdo_fetch("select * from ".$this->table_pe."lianhu_teacher where uid={$uid} and uniacid = {$_W['uniacid']} ");
        if($no && !$result) {
	...
	public function doMobileTeacher(){
		...
		if($_POST['submit']==1){
			...
			$tea_re = pdo_fetch("select * from ".$this->table_pe."lianhu_teacher where admin_id={$result['admin_id']} ");
			if($tea_re['uid']>0){
				$this->myMessage('该老师账号已经被绑定了',$this->createMobileUrl('teacher'),'错误');
			}
			$password = user_hash($_GPC['password'], $result['salt']);
			if($password == $result['password']){
				$re  = pdo_fetchall("select * from ".$this->table_pe."lianhu_teacher where uid={$uid} and uniacid = {$_W['uniacid']} ");
				if($re){
					$this->myMessage('您已经绑定教师，后台可以解绑',$this->createMobileUrl('teacher'),'错误');
				}
				$up['uid'] = $uid;
				pdo_update('lianhu_teacher',$up,array('teacher_id'=>$tea_re['teacher_id']));
	...
	public function mobile_from_find_student($to_bd=true){
		...
		$common_sql = "select stu.*, class.class_name ,grade.grade_name, tea.teacher_realname,tea.teacher_id 
					   ...
					   left join ".$this->table_pe."lianhu_grade grade on grade.grade_id=class.grade_id
			           left join  ".$this->table_pe."lianhu_teacher tea   on tea.teacher_id=class.teacher_id   where ";
    ...
    public function MoneyGive($arr=false){
        ...
        $num=0;
		$school_id = getSchoolId();
        $need_money_list=pdo_fetchall("select * from ".$this->table_pe."lianhu_money_limit where uniacid=:uniacid and school_id=:sid and status=1 ",array(':uniacid'=>$_W['uniacid'],':sid'=>$school_id ));
        if($need_money_list){
    ...
    public function getWechatMedia($media_id,$img_voice=1,$qiniu=true){
         ...
         if($img_voice==2){
            $up_file_name =$this->deAmr('attachment/'.$up_file_name);
		 }
         if($img){
             return $img;
		 }else{
    ...
    public function sendAllMsg($queue_id){
        ...
        $result = pdo_fetch("select * from ".tablename('lianhu_msg_queue')." where queue_id=:qid and end_time=0 and queue_status=1  ",array(':qid'=>$queue_id)); 
    ...
    public function deAmr($local_source){
         global $_W;
         $config = $this->module['config'];        
         $token = $this->checkJiaToken();
         if($token){
            $data['token']          =$token;
            $data['access_key']     =$config['qiniu_AccessKey'];  
            $data['secret_Key']     =$config['qiniu_SecretKey'];  
            $data['bucket']         =$config['qiniu_bucket'];  
            $data['pipeline']       =$config['qiniu_pipeline'];  
            ...
            $amr_de=$this->curlPost($this->jia_yun.'voice/amr2mp3',$data);
            $arr   =json_decode($amr_de,1);
            if($arr['errcode']==1){
                   $this->useJiaSend($arr['msg']);
			}
            return $arr['source_name'];
         }else{
            return false;
		 } 
    }
    ...
    public function getTeacherOneClass($teacher_id,$class_id){
        ...
        $week_today   =$this->decodeTodayWeek();
	...
	public function getAllVersion(){
			$list = pdo_fetchall("select * from ".$this->table_pe."lianhu_upgrade where 1=1"); 
			return $list;
	}
