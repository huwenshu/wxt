<?php 
    ...
    if(!$admin_result){
        ...
        if($_GPC['ac']=='change'  ){
            ...
            }elseif($_COOKIE[$_GPC['uniacid'].'_school_id']){
                setSchoolId($_COOKIE[$_GPC['uniacid'].'_school_id']);
            }else{
                $list = S('school','getSchoolByUniacid',array($_GPC['uniacid']) );
                setcookie($_GPC['uniacid'].'_school_id',$list[0]['school_id'],7*3600*24,'/');
        ...
        if($admin_result['admin']!='top'){
            isetcookie('__uniacid', $admin_result['uniacid'], 7 * 86400);
            isetcookie('__uid', $_W['uid'], 7 * 86400);            
            ...
            if($_GPC['_data_group_id']){
                if(count($admin_group_result['platform'])==1){
                    header("Location:".$this->createWebUrl('adminloginCheck',array('ac'=>'change','uniacid'=>$admin_group_result['platform'][0] ) ));
                }
