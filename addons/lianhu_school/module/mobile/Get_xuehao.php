...
if($_GPC['student_name']){
    ...
    if($class_name){
     ...
     $student_result = pdo_fetchall("select * from {$table_pe}lianhu_student where  student_name = :student_name and status=1 {$where}  and uniacid =:uniacid ",array(":student_name"=>$student_name,':uniacid'=>$_W['uniacid']) );
     if(!$student_result){
