...
#教师或者学生家长
    ...
    if($ac=='line_all'){
        $page           = $_GPC['page'];     
        $list           = $this->getLineList($page,5,$now_class_id);
    }else{
        $list           = $this->getLineList(1,5,$now_class_id);
    }
    foreach($list as $key=>$row){
            ...
            if($teacher_result){
                ...
                $list[$key]['member_img'] = $student_info['student_img'];
                $list[$key]['member_name']= $student_info['student_name'];
                $list[$key]['member_name_other'] = $row['relation'];
            }
            $list[$key]['member_img'] = $_W['attachurl'].$list[$key]['member_img'];
    }
    if($ac=='line_all'){
            if($list){
