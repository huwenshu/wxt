...
if($admin_result['admin']=='teacher'){
	$top_action = array(
		array('action_name'=>'导入成绩' ,'action'=>'data_in','arr'=>array('ac'=>'score','aw'=>1 ) ),
		array('action_name'=>'编辑成绩' ,'action'=>'Score_list' ),
	);
}else{
	$top_action = array(
		array('action_name'=>'导入年级 ','action'=>'data_in','arr'=>array('ac'=>'grade','aw'=>1 ) ),
		array('action_name'=>'导入班级' ,'action'=>'data_in','arr'=>array('ac'=>'class','aw'=>1 ) ),
		array('action_name'=>'导入教师' ,'action'=>'data_in','arr'=>array('ac'=>'teacher','aw'=>1 ) ),
		array('action_name'=>'导入学生' ,'action'=>'data_in','arr'=>array('ac'=>'student','aw'=>1 ) ),
		array('action_name'=>'补充学生资料' ,'action'=>'data_in','arr'=>array('ac'=>'student_data' ) ),
		array('action_name'=>'导入成绩' ,'action'=>'data_in','arr'=>array('ac'=>'score','aw'=>1 ) ),
		array('action_name'=>'考试记录' ,'action'=>'data_in','arr'=>array('ac'=>'score_list_jilv','aw'=>1 ) ),
		array('action_name'=>'新建考试记录' ,'action'=>'data_in','arr'=>array('ac'=>'score_list_jilv','aw'=>1,'op'=>'create' ) ),
	);
}
			...
			for($currentRow = $begin_in;$currentRow <= $allRow;$currentRow++){
				...
				for($currentColumn= 'A';$currentColumn<= $allColumn; $currentColumn++){
				   ...
				   if($currentColumn ==$grade_name){
				   		...
				   		if(!$have_or_not){
							   pdo_insert('lianhu_grade',array('grade_name'=>$grade_name_from_excel,'uniacid'=>$_W['uniacid'],'school_id'=>$school_id ));
							  $success++;
						}else{
