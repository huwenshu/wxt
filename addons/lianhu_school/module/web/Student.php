<?php 
	...
	$top_action = array(
		array('action_name'=>'添加学生','action'=>'student','arr'=>array('ac'=>'new','aw'=>1) ),
	);
	...
	if($ac=='list'){
                ...
                $num    		 =  $total;
                if($_GPC['print_code']==1 || $_GPC['excel'] || $_GPC['print_bd_code']==1){
					...
					if($_GPC['excel']){
								...
								$objPHPExcel->getProperties()->setCreator("家校通")->setLastModifiedBy("家校通")->setTitle("Office 2007 XLSX Test Document")->setSubject("Office 2007 XLSX Test Document")->setDescription("Test document for Office 2007 XLSX, generated using PHP classes.")->setKeywords("office 2007 openxml php")->setCategory("report file");
									$objPHPExcel->setActiveSheetIndex(0)->setCellValue('A1', '学生')->setCellValue('B1', '班级')->setCellValue("C1",'年级')->setCellValue("D1",'系统账号')->setCellValue("E1",'学号');
									$i=2;
									foreach ($list as $kv=> $v) {
										$objPHPExcel->setActiveSheetIndex(0)->setCellValue('A' . $i, $v['student_name'])->setCellValue('B' . $i, $v['class_name'])
										->setCellValue('C' . $i, $v['grade_name'])->setCellValue('D' . $i, $v['student_passport'])->setCellValue('E' . $i, $v['xuehao']);
										$i++;
									}
			...
			if($ac=='new'){
				if($_GPC['submit']){
					...
					$this->myMessage('新增成功',$this->createWebUrl('student', array('op' => 'student','aw'=>$aw )),'成功');
				}
			}
			if($ac=='edit'){
				$id 	= intval($_GPC['id']);
				...
				if($_GPC['submit']){
					...
					$this->myMessage('修改成功',$this->createWebUrl('student', array('op' => 'student','aw'=>$aw)),'成功');
				}
			}	
			if($ac=='delete'){
				$id         = intval($_GPC['id']);
				...
				if(!$result3){
					...
					$this->myMessage('删除成功',$this->createWebUrl('student', array('op' => 'student','aw'=>$aw)),'成功');
				}else{
					$this->myMessage('已有数据，无法删除',$this->createWebUrl('student', array('op' => 'student','aw'=>$aw)),'错误');
				}
			}		
