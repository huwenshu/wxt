<?php 
    ...
    if($_GPC['csv']){
        ...
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('A1', '学生')->setCellValue('B1', '班级')->setCellValue("C1",'年级')->setCellValue("D1",'时间');
        $i=2;
        foreach ($result['list'] as $kv=> $v) {
            ...
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue('A' . $i, $student_name)->setCellValue('B' . $i, $class_name)
                ->setCellValue('C' . $i, $grade_name)->setCellValue('D' . $i, $add_time);
                $i++;
       }
