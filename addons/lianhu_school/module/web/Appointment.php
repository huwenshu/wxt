<?php 
	...
	$top_action = array(
		array('action_name'=>'具体可预约列表','action'=>'appointment','arr'=>array('ac'=>'ac_list','aw'=>1 ) ),
		array('action_name'=>'新增可预约列表','action'=>'appointment','arr'=>array('ac'=>'ac_new','aw'=>1 ) ),
		array('action_name'=>'预约活动列表','action'=>'appointment','arr'=>array('ac'=>'list','aw'=>1 ) ),
		array('action_name'=>'新增预约活动','action'=>'appointment','arr'=>array('ac'=>'new','aw'=>1 ) ),
	);
	...
	if($ac=='list'){
		...
		$list  = pdo_fetchall("select * from {$table_pe}lianhu_appointment  where status!=3 {$school_uniacid} order by appointment_id desc {$sql_limit}");
	}
