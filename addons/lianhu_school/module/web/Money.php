<?php
	...
	if($ac=='edit'){
		...
		if($_GPC['submit']){
			...
			$this->myMessage('修改成功，请注意对于已经缴费过的用户本有效期内，这次修改不会起效，是否缴费是已ID值鉴别的；',$this->createWebUrl('money',array('aw'=>$aw)),'成功');
		}	
	...
	if($ac=='new'){
		if($_GPC['submit']){
			...
			$_GPC['limit_module'] = implode(',',$_GPC['limit_module']);
			$up = getNewUpdateData($_GPC,$cclass_moneyLimit->use_class);
			$cclass_moneyLimit->use_class->add($where,$up);
			$this->myMessage('新增成功','','成功');
		}
	}
	if($ac=='list'){
		$cclass_moneyLimit->use_class->each_page=10000;
		$re = $cclass_moneyLimit->use_class->getList(false);
		$list = $re['list'];
	}
