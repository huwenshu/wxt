<?php 	
        ...
        $top_action = array(
            // array('action_name'=>'发送记录','action'=>'msg','arr'=>array('aw'=>1,'op'=>'record') ),
        ...
        if($op    == 'msg_record'){
            ...
            if($_GPC['submit_weixin'] || $_GPC['submit_sms'] || $_GPC['submit_kf']){
                ...
                $where_common = " and status=1 and (fanid !='' or fanid1 !='' or fanid2 !='') " .$school_uniacid ;
                if($model == 'grade'){
                ...
                if($model == 'class'){
                    ...
                    $student_list    = pdo_fetchall("select * from {$table_pe}lianhu_student where class_id in({$class_id_str}) ".$where_common);				
                    $in['class_ids'] = $class_id_str;
                }
                if($model=='student'){
